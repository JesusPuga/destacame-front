...
export default {
  ...
  methods: {
    ...
    remove (object) {
      api.delete('passenger/' + object.id + '/').then(() => {
    ...
    create (data, id) {
      // update
      if (id) {
        api.patch('passenger/' + id + '/', data).then(response => {
          if (response.data) {
        ...
        api.post('passenger/', data).then(response => {
